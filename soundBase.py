...
class soundBase:
    ...
    def audioread(self, return_nbits=False, formater='sample'):
        ...
        """
        fs, data = wavfile.read(self.path)  # 修改为接收两个返回值
    
        if formater == 'sample':
            # 注意：此处假设原始数据为整型，且需要知道bits来转换到[-1,1]区间。
            # 但因为我们不再直接获得bits，你可能需要预先知道或硬编码位深度，
            # 或者从其他途径获取（如果该信息对你很重要的话）。
            # 以下代码假设bits为16，仅为示例，实际使用时请根据具体情况调整。
            max_int = 2**(16-1) - 1  # 假设16位精度
            data = data / max_int
